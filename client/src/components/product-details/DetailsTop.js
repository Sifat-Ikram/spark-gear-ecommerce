...
const DetailsTop = ({ product }) => {
  ...
  return (
    <section className="w-11/12 mx-auto max-w-[2000px] pt-5 pb-10">
      <nav
        className="flex flex-wrap items-center space-x-2 text-gray-600 text-xs sm:text-sm mb-6"
        aria-label="Breadcrumb"
      >
        <Link href="/" className="hover:text-primary roboto font-medium">
          Home
        </Link>
        <FaChevronRight className="w-3 h-3" />
        <Link
          href="/main-layout/allProduct"
          className="hover:text-primary roboto font-medium"
        >
          Products
        </Link>
        <FaChevronRight className="w-3 h-3" />
        <span className="text-gray-900 roboto font-semibold truncate sm:truncate-none max-w-[150px] sm:max-w-full">
          {product.name}
        </span>
      </nav>
      ...
      {product && (
        <div className="flex flex-col lg:flex-row lg:justify-between gap-10 items-center">
          <motion.div
            className="lg:w-2/5 xl:w-1/2 w-full flex-shrink-0"
          ...
          >
            <div className="flex-1 flex flex-col justify-between space-y-10 h-full">
              <div className="flex flex-col space-y-2">
                <h1 className="title-text exo font-bold text-gray-900 break-words leading-snug sm:leading-tight">
                  {product.name}
                </h1>

                <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-2 lg:space-x-3 2xl:space-x-5 space-y-1 sm:space-y-0">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm roboto text-gray-600">
                      {product.ratings.average} / 5 (
                      {product.ratings.reviewsCount} reviews)
                    </span>
                  </div>
                  <h1 className="text-gray-700">||</h1>
                  <span className="text-sm roboto text-gray-700">
                    {product.warranty} warranty available
                  </span>
                </div>
              </div>

              <p className="text-sm sm:text-base xl:text-lg 2xl:text-xl roboto text-gray-600 text-left w-4/5">
                {product?.shortDescription}
              </p>

              <div className="flex flex-col space-y-4 text-gray-800 text-sm sm:text-base xl:text-lg 2xl:text-xl roboto">
                <span>
                ...
                <span className="text-lg 2xl:text-xl roboto font-semibold">
                  {product?.price} BDT
                </span>
              </div>
            </div>
